...
def get_db_connection():
    return psycopg2.connect(
        ...
        database=os.getenv("POSTGRES_DB", "book_reviews"),
    ...
    try:
        with conn.cursor() as cur:
            ...
            if not check_table_exists(conn, "conversations"):
                ...
                """)
                if DEBUG:
                    print(f'Table `conversations` created.')

            if not check_table_exists(conn, "feedback"):
                cur.execute("""
                ...
                """)
                if DEBUG:
                    print(f'Table `feedback` created.')
        conn.commit()
    finally:
        conn.close()
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("""
                ...
                RETURNING timestamp;
            """, 
            ('test', 'test question', 'test answer', 'test category', 'test model', 0.0, 0.0, 
